...
or 3-D fields. The function also handles arbitrary phase shifts in the input
and output arrays.
"""
...
class transform():
    ...
    def get_inverse_transform(self):
        ...
        Returns:
            An instance of a derivative of the transform class that performs
                the inverse transformation of the caller class.
        """
        print "Must overwrite the transform class get_inverse_transform " + \
            "function!"
        pass
    ...
    def _check_data(self, data):
        """
        Generic transform class _check_input method. **Must be implemented by
        all children.**
    ...
    def _fourier(self, data):
        """
        The global fourier method that handles all regularly gridded fourier
        (and inverse fourier) transformations and any required phase shifting.
        ...
        for i in range(self.ndim):
            if self.in_ref[i] != 0. and self.ft_type[i] != NONE:
                shift = np.exp(self.ft_type[i] * np.pi * ii *
                               self.in_ref[i] *
                               (self.out_axes[i] + self.out_ref[i]))
                # I think this works...
                idx_obj = [None] * self.ndim
                idx_obj[i] = slice(None)
    ...
    def _parse_ft_type_arg(self, ft_type):
        ...
        do_fft = False
        do_ifft = False
        # if you give an empty list to fftn in the axes position, nothing
        # happens
        fftaxes = []
        ifftaxes = []
...
def gfft(inp, in_ax=[], out_ax=[], ftmachine='fft', in_zero_center=True,
    ...
    in_ax, out_ax: The axes on which the input/output arrays are defined. There
        ...
        To go from irregularly spaced input to irregularly spaced output: This
            ...
            output arrays live on a grid. The grid can be defined either for
            the input or output space (which is why either in_ax or out_ax can
            be given as a tuple).
    ...
    ftmachine: a length N list of strings, with each entry containing either
        ...
        performed for each axis. So, if you have a 3D dataset and you want to
        do an FFT on the first two axes, but an IFFT on the last, you would
        pass ftmachine=['fft', 'fft', 'ifft']. In principle, we could also make
        DFTs an option here, and the code would just do a DFT rather than
        gridding. For an N-D input array, one could also just use
        ftmachine='fft' and it would do an fft for all axes.

        For now, options include: 'fft', 'ifft', and 'none'.

    in_zero_center/out_zero_center: a length N list of booleans. True indicates
        that the zero frequency is in (or should be in) the central pixel,
        false indicates that it is in pixel 0. Basically this indicates whether
        fftshifts should be performed before and after Fourier transforming.
        For an N-D array, in_zero_center=T would indicate that all axes should
        have the zero channel in the central pixel.
    ...
    if len(in_ax) == 0:
        ...
        if len(out_ax) != 0:
            warnings.warn('in_ax is empty, indicating regular to regular ' +
                          'transformation is requested, but out_ax is not ' +
                          'empty. Ignoring out_ax and proceeding with ' +
                          'regular to regular mode.')
...
# perform a shift.
    if (do_fft is False and do_ifft is False) or \
        (fftaxes == [] and ifftaxes == []):
            warnings.warn('No Fourier transformation requested, only ' +
                          'shifting will be performed!')
#            return
            mode = MODE_RR  # Since gridding will not be needed, use RR mode
    ...
    if verbose:
        ...
        for i in range(N):
            ...
            if fftaxes is None or fftaxes.count(i) > 0:
                pstr = pstr + 'True, '
            else:
                pstr = pstr + 'False, '

            if ifftaxes is None or ifftaxes.count(i) > 0:
                pstr = pstr + 'True, '
            else:
                pstr = pstr + 'False, '

            if preshift_axes is None or preshift_axes.count(i) > 0:
                pstr = pstr + 'True, '
            else:
                pstr = pstr + 'False, '

            if postshift_axes is None or postshift_axes.count(i) > 0:
                pstr = pstr + 'True, '
            else:
                pstr = pstr + 'False, '
    ...
    # Do MODE_IR transform
        ...
        if N == 1:
            ...
            if do_preshift:
                if preshift_axes is None:  # shift all axes
                    vmin = -0.5 * Nv * dv
                    umin = -0.5 * Nu * du
                    ...
                    if preshift_axes.count(0) > 0:
                        umin = -0.5 * Nu * du
            if do_postshift:
                if postshift_axes is None:  # shift all axes
                    xmin = -0.5 * Nx * dx
                    ymin = -0.5 * Ny * dy
                else:  # only shift some axes...
    ...
    # Do MODE_II transform
        ...
        if type(in_ax) == tuple:
            ...
            if N == 1:
                ...
                Nx = in_ax[1][0][1]
                xmin = 0.
                if type(in_zero_center) == bool and in_zero_center is True:
                    xmin = -0.5 * Nx * dx
                elif in_zero_center[0] is True:
                    xmin = -0.5 * Nx * dx
                du = 1. / dx / Nx / alpha
                Nu = alpha * Nx
                umin = 0.
...
#            print alpha * *2*Nx
        ...
        if N == 1:
            ...
            out_degrid = gridding.degrid_2d(out_ax[0], out_ax[1], out,
                                            dx / alpha, alpha ** 2 * Nx,
                                            alpha * xmin, dy / alpha,
                                            alpha ** 2 * Ny, alpha * ymin,
                                            alpha, W)

        elif N == 3:
            out_degrid = gridding.degrid_3d(out_ax[0], out_ax[1], out_ax[2],
                                            out, dx / alpha, alpha ** 2 * Nx,
                                            alpha * xmin, dy / alpha,
                                            alpha ** 2 * Ny, alpha * ymin,
                                            dz / alpha, alpha ** 2 * Nz,
                                            alpha * zmin, alpha, W)
...
def dft(in_vals, in_ax, out_ax):
    ...
    nax = len(in_ax)
    if len(out_ax) != len(in_ax):
        raise Exception('dft: number of input and output dimensions ' +
                        'not equal!')
